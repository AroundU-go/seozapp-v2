...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    try {
      let query = supabaseV2Admin.from(V2_TABLES.AEO_AUDITS).select('*').order('created_at', { ascending: false }).limit(50);
      if (userEmail) {
  ...
  if (req.method === 'POST') {
    ...
    if (!planLimits.isPro) {
      return res.status(403).json({ error: 'Active subscription required for AI Citation & AEO Audit. Please upgrade your plan.' });
    }
